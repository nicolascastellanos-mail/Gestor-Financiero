...
def import_function():
	...
	file_label = ttk.Label(import_root, text="File name")
	file_label.grid(row=0, column=0, padx=10, pady=10)

	file_entry = ttk.Entry(import_root)
	file_entry.grid(row=0, column=1, padx=10, pady=10)

	buttons_frame = ttk.Frame(import_root)
	buttons_frame.grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=10, pady=10)

	accept_button = ttk.Button(buttons_frame, text="Aceptar", command=lambda:load(file_entry.get()))
	accept_button.grid(row=1, column=0, padx=10, pady=10, sticky=tk.W)

	cancel_button = ttk.Button(buttons_frame, text="Cancelar", command=import_root.destroy) # TODO fix command
	cancel_button.grid(row=1, column=1, padx=10, pady=10, sticky=tk.W)
...
import_ = ttk.Button(buttons, text="Importar", command=import_function)
